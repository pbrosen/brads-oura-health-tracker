---
name: health-log
description: Log food, drinks, exercise, or digestive (gut) symptoms to Nila's daily Health Tracker files, and answer questions about today's intake / burn / macros / gut symptoms. ALWAYS trigger this skill when Nila says she ate, drank, had, finished, or logged any food or drink ("I had a turkey sandwich for lunch", "log a Diet Coke", "two eggs and toast at 7:30"), when she logs exercise ("did 30 min weight training", "lifted for 45 min", "log my run"), when she logs a gut/digestive episode ("log a bout of diarrhea", "loose stool this morning", "bloated after lunch, bristol 6"), or when she asks about today's nutrition, burn, or gut history ("what have I eaten so far", "what's my net calorie balance", "how many calories left", "what foods might be triggering my stomach"). Also trigger on past-day variants ("log yesterday's dinner") and edits ("change lunch to 600 calories", "remove the Diet Coke"). Also trigger when she asks to sync/refresh/pull her Oura data ("run my Oura import now", "sync my Oura", "pull today's Oura numbers"). This skill is the canonical way to manipulate the tracker when the Health Tracker dashboard isn't open — chat, mobile, and scheduled tasks all go through it to keep the file format consistent.
---

# Health log

Nila runs a daily food + exercise + gut-symptom tracker. The dashboard is a Cowork
artifact ("Health Tracker"), but the underlying data lives as **plain Markdown (and one
JSON) files in a local folder on Nila's computer**. This skill writes to and reads from
those files directly — the same file format the dashboard uses, so dashboard and
chat-driven logging stay consistent.

This is the non-Obsidian, non-Strava build of the system: there is **no Obsidian and no
vault**, and daily activity comes from an **Oura ring** via a scheduled import task rather
than a live Strava connector. Use the normal file tools (Read / Write / Edit) on the local
folder.

## Where the files live

Everything lives under Nila's Health Tracker folder, which is the folder she connected
...
- **Food library:** `BASE/food-log/_library.md` — personal database of foods Nila has
...
- **Oura daily activity:** `BASE/oura/YYYY-MM-DD.json` — one JSON file per day, written by
  Nila's own scheduled import task (see "Oura" below). Read-only from this skill's normal
  logging workflow — only the import step (or the dashboard's "↻ Import Oura" button)
  writes here.

To find `BASE`: it is the folder Nila connected to this Cowork session. If you're unsure
...
- `Read` — read one file. If it doesn't exist, that's expected: treat a missing day file
  ...
  and a missing Oura file as "no Oura data synced for that day yet."
...
- _(Oura-derived burn + activity minutes are synced nightly and not duplicated here.)_
...
dashboard so historical days don't shift when Nila later changes her settings. **Preserve
...
1. **Resolve the date.** Default = today. Accept "yesterday", "Monday", explicit dates.
   Convert to `YYYY-MM-DD` in Nila's local timezone.
2. **Resolve the time.** Default = current local time formatted `HH:MM` (24-hour). If Nila
...
4. **Check the library** (see "Library handling"). If there's an exact normalized-name
   ...
   than a fresh estimate because it reflects Nila's prior corrections.
5. **If no library hit, estimate the macros yourself.** Use realistic restaurant/home
   values. Respect any quantity Nila gave ("2 eggs", "large", "8 oz"); otherwise assume one
...
Valid `source` values: `manual` (Nila supplied the macros), `library` (matched a known
food), `ai` (you estimated), `edited` (corrected after the fact).

## Workflow: logging an exercise entry

The `exercise` array is for activity Oura does **not** capture as a distinct workout —
weight training, yoga, etc. Oura's daily numbers (active calories, steps, high-activity
minutes) are whole-day totals, not per-workout entries, so there's no risk of duplicating
a specific session the way Strava runs/rides could be. Same date/time resolution as food,
then:

1. Get from Nila: name (required), calories burned (required — if she doesn't know,
   estimate from duration + activity type), duration in minutes (optional), VO₂max zone
...
When Nila asks "what foods might be triggering my stomach / gut / diarrhea":

1. Read `symptom-log.md`; keep episodes of type `diarrhea` and `loose`. Need at least ~2
   to say anything useful; if fewer, tell her to keep logging.
...
4. Report suspects as **leads to watch, not proof**. State plainly: correlation isn't
   ...
   correlations" button that does exactly this — point Nila there for the interactive view.)

## Oura (read from local JSON files — no live connector)

There is no Oura connector in Cowork's directory, so Nila's Oura data does **not** come
from a live tool call the way Strava would. Instead, a scheduled task she set up during
install (see `references/oura-import.md` and the plugin's `README.md` / `CONNECTORS.md`)
calls the Oura API directly, once a night, and writes one JSON file per day to
`BASE/oura/YYYY-MM-DD.json`. **Read that file — don't call the Oura API yourself in normal
chat logging or reporting.**
...
```json
{
  ...
  "total_calories": 2410,
  "steps": 8214,
  "high_activity_minutes": 42,
  "readiness_score": 78
}
```

- `active_calories` — counts toward burn/net, same role Strava's per-activity kcal played.
- `high_activity_minutes` — the closest thing Oura exposes to "VO₂ zone minutes." It is
  **not** a lab VO2max measurement — when you report it, call it "activity minutes" or
  "high-activity minutes (from Oura)," not "VO2max zone minutes."
- `steps`, `readiness_score` — extra context; mention them if Nila asks, not by default.

### Reporting burn / net

When Nila asks about burn or net for a date:

1. Sum food → intake calories + macros from the day file.
2. Read `BASE/oura/<date>.json` if it exists. `active_calories` adds to burn;
   `high_activity_minutes` adds to the activity-minutes total.
3. Add manual exercise calories from the day file.
4. Add Nila's prorated resting burn (BMR + NEAT math below).
5. Net = intake − total burn.

If that day's Oura file doesn't exist yet (nightly sync hasn't run, or it's a fresh
install), report intake + manual exercise + resting burn, and note that today's Oura
number isn't in yet — offer to run the import now (see below).

### Running the Oura import on demand

If Nila says "run my Oura import now", "pull my Oura data", "sync Oura", or similar:

1. Prefer triggering her scheduled task (named exactly `oura-daily-import` — this must
   match what she named it during setup) if you have a way to invoke it from chat.
2. Otherwise, do what that task does directly: call the Oura API's `daily_activity` and
   `daily_readiness` endpoints for the target date (default today, auth header
   `Bearer <her token>`, base URL `https://api.ouraring.com/v2/usercollection/`) and write
   `BASE/oura/YYYY-MM-DD.json` in the shape above.
3. Confirm briefly with the numbers you wrote.

Note: the dashboard has its own "↻ Import Oura" button that calls
`window.cowork.runScheduledTask("oura-daily-import")` directly and polls for the new file —
that covers the case where Nila is looking at the dashboard. This chat path covers the case
where she asks you directly instead.

### Nila's stats (for BMR / resting calorie math)

These live in the dashboard's **Calorie-burn settings** (weight, height, age, default NEAT)
and are the source of truth. If you don't know them, ask Nila once and remember them for the
session.

- BMR (Mifflin–St Jeor, male): `10 × weight_kg + 6.25 × height_cm − 5 × age + 5` kcal/day.
  (For female, the constant is `− 161` instead of `+ 5`. Confirm with Nila if unsure.)
- **NEAT** (non-exercise activity thermogenesis — walking, stairs, errands): a flat kcal/day
  added to BMR. Default ≈ 350; Nila can override it globally in Settings or per-day in the
...
If Nila mentions a weight change in chat, remind her to update it in the dashboard's
...
2. **Preserve resting-burn fields.** If the file you read had `bmr_per_day` / `neat_kcal`,
   ...
   line, and the DATA block). If it didn't have them, you may compute `bmr_per_day` from Nila's
   stats and use the default `neat_kcal` (≈350) — or simply omit both; the dashboard will fill
   them the next time Nila opens it. Never *drop* values that were already there.
...
`updated` to today and keep `original_name` as whatever Nila originally typed (case preserved).

## Confirming back to Nila

After a successful write, confirm briefly — she just wants to know it landed:
...
For an Oura import: `Pulled today's Oura data: 320 active cal · 8,214 steps · 42 high-activity min · readiness 78.`

## First-time setup

If the Health Tracker folder is empty / files don't exist yet, or Nila asks to "set up my
Health Tracker":

1. Make sure the **Filesystem desktop extension** is installed and pointed at her data
...
2. Create the dashboard artifact from the bundled HTML at
   ...
   wire it to her Filesystem connector:** check which filesystem tools are actually connected
   ...
   `filesystem`; if hers differs, that's the one thing to fix. The dashboard then
   auto-discovers the data folder via `list_allowed_directories`.
3. The first time she opens the dashboard it greets her with a short setup wizard (weight,
   height, age, and an everyday-activity level that sets her NEAT). Once she completes or skips
   it, it won't show again; she can always change these numbers in **Calorie-burn settings**.
4. Point her at `references/oura-import.md` (also covered in the plugin's `CONNECTORS.md`) to
   get her Oura personal access token and create the nightly import task — **it must be named
   exactly `oura-daily-import`** so the dashboard's "↻ Import Oura" button and the "run my Oura
   import now" chat path both work. This also requires enabling network access to
   `api.ouraring.com` in Settings → Capabilities → Code execution (domain allowlist) — flag
   this explicitly, it's the step people miss.
5. Tell her she can now log by chatting (this skill) or in the dashboard, and that her Oura
   numbers appear once the nightly sync (or an on-demand "run my Oura import now") has written
   today's file.

## Things that should NOT happen

- Don't call the Oura API during normal food/exercise/symptom logging — that only happens via
  the scheduled import task or an explicit "run my Oura import now" request.
- Don't break the DATA block JSON. Parse it back before writing.
- Don't silently overwrite a day's entire log. Append; only consolidate if Nila asks.
...
- Don't try to write the dashboard's stored Settings (weight/height/age/NEAT) — those are in
  the dashboard's local storage; ask Nila to change them in the Settings panel.
- Don't overclaim precision on `high_activity_minutes` — it's Oura's own metric, not a
  clinical VO2max reading.
